...
def open_new_window(device):
    ...
    ip_config_button = tk.Button(new_window, text="IP Configuration", command=lambda :ip_config_command(device))
    ip_config_button.pack(pady=10)

    # Add Command Prompt button
    command_prompt_button = tk.Button(new_window, text="Command Prompt", command=lambda : command_prompt_command(device))
    command_prompt_button.pack(pady=10)

    # Add Web Browser button
    web_browser_button = tk.Button(new_window, text="Web Browser", command=lambda :web_browser_command(device))
    web_browser_button.pack(pady=10)

    # Add Email button
    email_button = tk.Button(new_window, text="Email", command=lambda :email_command(device))
    email_button.pack(pady=10)

    # Add Text Editor button
    text_editor_button = tk.Button(new_window, text="Text Editor", command=lambda:text_editor_command(device))
    text_editor_button.pack(pady=10)

    # Add Files Manager button
    files_manager_button = tk.Button(new_window, text="Files Manager", command=lambda : files_manager_command(device))
    files_manager_button.pack(pady=10)

    # Add About button
    about_button = tk.Button(new_window, text="About", command=lambda :about_command(device))
    about_button.pack(pady=10)

def ip_config_command(device):
    print(f"IP Configuration", f"This is the IP Configuration command by {device.name}")

def command_prompt_command(device):
    print(f"Command Prompt", f"This is the Command Prompt command by {device.name}")
    # messagebox.showinfo()
